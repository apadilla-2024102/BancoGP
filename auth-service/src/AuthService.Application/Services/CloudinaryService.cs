...
public class CloudinaryService(IConfiguration configuration) : ICloudinaryService
{
    ...
    {
        var defaultPath = configuration["CloudinarySettings:DefaultAvatarPath"] ?? "bancogp_xjoycv.png";
    ...
    {
        var baseUrl = configuration["CloudinarySettings:BaseUrl"] ?? "https://res.cloudinary.com/dbjg9o5oj/image/upload/v1771474381";
        var folder = configuration["CloudinarySettings:Folder"] ?? "BancoGP/Profile";
        var defaultPath = configuration["CloudinarySettings:DefaultAvatarPath"] ?? "bancogp_xjoycv.png";
    ...
    public async Task<string> UploadImageAsync(IFileData imageFile, string fileName)
    {
        try
        {
            ...
            var folder = configuration["CloudinarySettings:Folder"] ?? "BancoGP/profiles";
